...
//        System.out.println(labor.get(1).get());
    ...
    public void create(Entitie.Labor labor) {
        String query = "INSERT INTO labors (name, unitcost, quantity, type,  hourlyrate, hoursworked, workerproductivity,project_id) VALUES (?, ?, ?, CAST(? AS componenttype), ?, ?, ?, ?)";
        try {
            int rowsAffected = database.executeUpdate(query, labor.getName(), labor.getUnitCost(), labor.getQuantity(), "Labor", labor.getHourlyRate(), labor.getHoursWorked(), labor.getWorkerProductivity(), labor.getProject().getId());
            if (rowsAffected > 0) {
    ...
    public void update(int id, Entitie.Labor labor) {
        String sql = "UPDATE labors SET name = ?, unitcost = ?, quantity = ?, type = CAST(? AS componenttype), hourlyrate = ?, hoursworked = ?, workerproductivity = ?, project_id = ? WHERE id = ?";
        try {
            int rowsAffected = database.executeUpdate(sql, labor.getName(), labor.getUnitCost(), labor.getQuantity(), "Labor", labor.getHourlyRate(), labor.getHoursWorked(), labor.getWorkerProductivity(), labor.getProject().getId(), id);
            if (rowsAffected > 0) {
